...
Autor: Dashboard I&C
Fecha: Febrero 2026
Versión: 1.9.3 - Fix: session_state Hito S + sorted() TypeError en columnas mixtas
...
def cargar_datos(url_excel):
    ...
    try:
        ...
        # Formatear Hito S: 1.0→"1", 1.5→"1.5"
        if 'Hito S' in df.columns:
            def _fmt_hito(x):
                try:
                    n = float(x)
                    if n != n: return x           # NaN check
...
if df is not None:
    ...
    with st.sidebar:
        ...
        # Limpiar session_state Hito S si tiene valores obsoletos (floats vs strings)
        if 'Hito S' in df.columns:
        ...
        def _safe_sort(vals, col=None):
            """Ordena lista de valores mixtos sin TypeError."""
            if col == 'Hito S':
                def _sk(v):
