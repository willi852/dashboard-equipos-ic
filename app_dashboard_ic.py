...
Versión: 1.0.9 - Fix: Pendientes Suministro de Aire excluye N/A correctamente
...
def calcular_completados(df, actividad):
    ...
    if 'suministro' in actividad.lower() or 'suiministro' in actividad.lower():
        ...
        df_aplicable = df[~df[actividad].isin(VALORES_NA) & df[actividad].notna()].copy()
        total = len(df_aplicable)
        if total == 0:
            return 0, 0, 0, 0
        completados = int(df_aplicable[actividad].isin(VALORES_OK).sum())
    ...
    try:
        valores_completados = df[actividad].isin(VALORES_OK).sum()
...
if df is not None:
    ...
    # Panel informativo para Suministro de Aire
    # NOTA: se usan variables intermedias para evitar SyntaxError en Python 3.11+
    ...
    if actividades_seleccionadas:
        ...
        for actividad in actividades_seleccionadas:
            if 'suministro' in actividad.lower() or 'suiministro' in actividad.lower():
                # ✅ CORRECTO: pendiente = SÍ aplica (no N/A, no vacío) AND NO completado
                mask_aplica     = ~df_pendientes_combinado[actividad].isin(VALORES_NA) & df_pendientes_combinado[actividad].notna()
                mask_incompleto = ~df_pendientes_combinado[actividad].isin(VALORES_OK)
                df_pendientes_combinado[f'{actividad}_Pendiente'] = mask_aplica & mask_incompleto
            else:
                df_pendientes_combinado[f'{actividad}_Pendiente'] = (
                    ~df_pendientes_combinado[actividad].isin(VALORES_OK)
                )
    ...
    with tab1:
        if 'AREA' in df_filtrado.columns:
            ...
            with col_a1:
                fig_area = px.pie(
                    analisis_area, values='Cantidad', names='AREA',
